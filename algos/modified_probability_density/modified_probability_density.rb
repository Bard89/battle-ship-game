...
module ModifiedProbabilityDensity
  ...
  def update_probabilities_after_firing(target_row, target_col, result, probability_grid, api)
    hit = result['cell'] == 'X'
    # update_adjacent_cells(probability_grid, target_row, target_col, hit)
    update_hit_or_miss_probability(probability_grid, target_row, target_col, hit)
    ...
    # uncomment to develop further ( the constraint_solver algo solves this problem properly )
    # update_ship_sunk_or_not(probability_grid, target_row, target_col, hit, api)
  end
