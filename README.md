...
**IMPORTANT and MOST USEFUL -> ** To run the solver just run `ruby run_battleship_mock.rb`
...
One of those ships is then irregular and has 4 "chimneys" on bots sites of the ship.
This irregular ship is carrying "avengers" with special abilities.

## The Solver

The solver is fully functional except for the API and Avengers. Unfortunately the API was disabled before I could
finish the project. That prevented me from continuing, because the API calls and responses are not properly documented.
( see the original copy of the webpage )

To run the solver just run `ruby run_battleship_mock.rb`

### The approach to solve the game
...
### The little I managet to pull out of the API before it was shut down. 
